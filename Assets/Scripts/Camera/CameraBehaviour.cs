...
public class CameraBehaviour : MonoBehaviour
{
    ...
    void FixedUpdate()
    {
        if (m_target != null)
        {
            ...
            float camSmoothX = transform.position.x + ((target.x - transform.position.x) * 0.3f) * _cameraFollowSpeed * Time.fixedDeltaTime;
            float camSmoothY = transform.position.y + ((target.y - transform.position.y) * 0.6f) * _cameraFollowSpeed * Time.fixedDeltaTime;
